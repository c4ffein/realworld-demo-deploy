...
token_scheme = APIKeyHeader(
    ...
    scheme_name="Token",
    description=(
        "For accessing protected API resources, you must have received a valid JWT token "
        "after registering or logging in. This JWT token must then be used for all protected "
        "resources by passing it in via the 'Authorization' header.\n\n"
        "A JWT token is generated by the API by either registering via /users or logging in "
        "via /users/login.\n\n"
